...
class Amenu extends CWidget
{
	...
	protected function renderMenuItem($item)
	{
		if(isset($item['url']))
		{
			$label=$this->linkLabelWrapper===null ? $item['label'] : CHtml::tag($this->linkLabelWrapper, isset($item['htmlOptions']) ? $item['htmlOptions'] : array('class' => 'btn'), '').' '.$item['label'];
			return CHtml::link($label,$item['url'],isset($item['linkOptions']) ? $item['linkOptions'] : array('class' => 'btn btn-primary btn-sm btn-flat'));
		}
		else
			return CHtml::tag('i',isset($item['linkOptions']) ? $item['linkOptions'] : array(), $item['label']);
	}
