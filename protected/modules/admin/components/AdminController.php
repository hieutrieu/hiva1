...
class AdminController extends CController
{
	...
	public function renderButton() {
		if(count($this->button)) {
			$this->beginWidget('zii.widgets.CPortlet', array('htmlOptions' => array('class' => 'toolbar-list'),'contentCssClass' =>'', 'id'=> 'toolbar'));
			$this->widget('Amenu', array(
				'items' => $this->button,
				'htmlOptions'=>array('class' => 'nav_button'),	
				'linkLabelWrapper' => 'i',
				'itemCssClass' => '',
			));
			$this->endWidget();	
	...
	public function renderSubmenu() {
		if(count($this->subMenu)) {
			$this->beginWidget('zii.widgets.CPortlet', array('htmlOptions' => array('class' => null, 'id' => 'submenu-box'),'contentCssClass' => 'm', 'id' => ''));
			$this->widget('zii.widgets.CMenu', array(
				'items' => $this->subMenu,
				'htmlOptions'=>array('id' => 'submenu'),				
			));
			echo CHtml::tag('div', array('class' => 'clr'), '');
			$this->endWidget();			
		} else {
			return false;
		}
	...
	public function t($messages) {
		return Yii::t('app', $messages);
	}
	
	public function init() {
		if(isset($_GET['tmpl']) && $_GET['tmpl'] == 'modal') {
			$this->layout = 'webroot.themes.admin.modal';
			$this->pageAjax = true;
			$this->function = Yii::app()->request->getParam('function', 'mWaring');			
		}	
	}
