...
class Category extends CActiveRecord
{
	...
	public static function getChilds($id) {
		...
		foreach(Category::model()->findAll('parent_id = ' . $id) as $model) {
			$row['text'] = CHtml::link($model->title, array('category/view', 'id' => $model->id));
	...
	public function rules()
	{
		return array(
			...
			array('id, parent_id, title, description, language', 'safe', 'on'=>'search'),
		);
	}
	...
	{
		return array(
			'Products' => array(self::HAS_MANY, 'Products', 'id'),
